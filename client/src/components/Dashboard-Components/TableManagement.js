        ...
        <tbody>
          {
            tables.map(tables => (
                <tr key={tables._id}>
                    ...
                    <td>
                    <Link to={`/dashboard/edittabledetails/${tables._id}`} className="btn btn-primary" style={{marginRight: 5}}>Edit</Link>
                    <button type="button" className="btn btn-danger" onClick={() => deleteTableDetails(tables._id)}>Delete</button>  
                    </td>
                </tr>
            ))
          }
